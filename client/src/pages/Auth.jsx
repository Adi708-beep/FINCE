...
import { Briefcase, Scan, FileText, Loader2, TrendingUp, Percent } from "lucide-react";
...
const Auth = ({ mode = "login" }) => {
  ...
  // Corporate OCR Simulator States
...
TOTAL AMOUNT DUE             ₹19,470.00
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      // In our server, role can be personal or business. We call login without specifying it
      // so it validates credentials globally and returns the correct active role!
  ...
  const handleRegisterSubmit = async (e) => {
    ...
    try {
      const mappedRole = registerData.userMode === "corporate" ? "business" : "personal";
      await register(
        registerData.email.split("@")[0], // username fallback
        registerData.email,
        registerData.password,
        mappedRole,
        registerData.fullName,
      ...
      setMessage({
        type: "success",
        text: "Registration successful! Loading your workspace...",
      });
      setTimeout(() => {
        navigate("/dashboard");
      }, 1500);
    ...
    <div className="relative min-h-screen w-full flex items-center justify-center bg-slate-50 overflow-y-auto py-12 px-4 selection:bg-purple-100 selection:text-purple-900">
      ...
      <div 
        className={`bg-white/80 border border-slate-200/60 rounded-3xl p-8 w-full shadow-2xl relative z-10 hover:border-purple-500/25 transition-all duration-500 ease-in-out backdrop-blur-xl animate-scale-up ${
          isLogin ? "max-w-md" : (!isLogin && registerData.userMode === "corporate" ? "max-w-5xl" : "max-w-lg")
        }`}
      >
            ...
            >
              {registerData.userMode === "corporate" ? (
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                  ...
                  {/* Left Column: Business Overview & Simulator Page */}
                  <div className="lg:col-span-6 space-y-5 lg:border-r lg:border-slate-100 lg:pr-6 text-left">
                    <div className="space-y-2">
                      <span className="px-2.5 py-1 rounded-full text-[9px] font-bold tracking-wider uppercase bg-blue-50 border border-blue-200 text-blue-600 inline-block font-sans">
                        Enterprise Ledger Gateway
                      </span>
                      <h2 className="text-xl font-black tracking-tight text-slate-900 font-outfit leading-tight">
                        Automate Corporate Accounts Payable
                      </h2>
                      <p className="text-slate-500 text-xs leading-relaxed font-medium">
                        Upload vendor invoices and corporate bills. FINCE automatically extracts critical billing structures like invoice numbers, GSTIN, due dates, taxes, and triggers duplicate checks & warnings instantly.
                      </p>
                    </div>

                    {/* OCR Simulator Panel */}
                    <div className="bg-[#F8FAFC]/90 border border-slate-200/80 p-4.5 rounded-2xl relative overflow-hidden shadow-sm">
                      <div className="flex justify-between items-center mb-3 border-b border-slate-150 pb-2">
                        <div className="flex items-center gap-1.5">
                          <Scan className="w-3.5 h-3.5 text-blue-600" />
                          <span className="text-xs font-bold font-outfit text-slate-800">Accounts Invoice Simulator</span>
                        </div>
                        {simStatus === 'idle' && (
                          <button 
                            type="button"
                            onClick={runSimulation}
                            className="px-3 py-1 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-extrabold text-[9px] tracking-wider uppercase rounded-lg shadow-sm hover:shadow-md transition-all cursor-pointer border-none"
                          >
                            Simulate OCR
                          </button>
                        )}
                        {simStatus !== 'idle' && simStatus !== 'done' && (
                          <div className="flex items-center gap-1 text-[9px] text-blue-600 font-mono animate-pulse">
                            <span className="w-1 h-1 bg-blue-600 rounded-full animate-ping mr-1" />
                        ...
                        {simStatus === 'done' && (
                          <button 
                            ...
                            className="text-[9px] text-slate-400 hover:text-slate-600 underline font-bold border-none bg-transparent cursor-pointer"
                          >
                            Reset Demo
                          </button>
                        )}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 h-48">
                        {/* Receipt Visualizer */}
                        <div className="bg-white rounded-xl border border-slate-200 p-3 font-mono text-[8px] text-slate-500 relative overflow-hidden select-none">
                          {simStatus === 'scanning' && (
                            <div 
                              className="absolute left-0 w-full h-0.5 bg-blue-500 shadow-md z-10"
                              style={{ 
                                top: `${simProgress}%`,
                          ...
                          <div className={simStatus === 'scanning' ? 'opacity-40' : 'opacity-100 transition-opacity duration-300'}>
                            {mockInvoice.trim().split('\n').map((line, idx) => (
                        ...
                        <div className="bg-white rounded-xl border border-slate-200 p-3 flex flex-col justify-center items-center">
                          {simStatus === 'idle' && (
                            <div className="text-center space-y-1.5 p-1">
                              <FileText className="w-5 h-5 text-slate-400 mx-auto" />
                              <p className="text-[9px] text-slate-400 font-semibold leading-relaxed">
                                Click the simulator button to parse corporate details into ledger categories.
                              </p>
                            </div>
                          )}
                          ...
                          {simStatus === 'analyzing' && (
                            <div className="text-center space-y-1">
                              ...
                              <p className="text-[9px] text-blue-600 font-bold font-mono">GEMINI INGESTION ENGAGED...</p>
                            </div>
                          )}

                          {simStatus === 'done' && (
                            <div className="w-full h-full flex flex-col justify-between text-[10px] space-y-1">
                              <div className="flex justify-between items-center border-b border-slate-100 pb-0.5">
                                <span className="font-bold text-slate-800 text-[10px] truncate max-w-[80px]">AWS Inc.</span>
                                <span className="px-1.5 py-0.2 rounded-full text-[7px] bg-blue-50 text-blue-600 border border-blue-100 font-bold">Cloud Infra</span>
                              </div>

                              <div className="space-y-0.5 flex-1 overflow-y-auto font-mono text-slate-500 text-[8px] scrollbar-thin">
                                <div className="flex justify-between text-slate-400">
                                  <span>Inv No: AWS-902482</span>
                                </div>
                                <div className="flex justify-between pt-0.5">
                                  <span>EC2 Cloud</span>
                                  <span className="text-slate-800">₹8,500</span>
                                </div>
                                <div className="flex justify-between">
                                  <span>Aurora DB</span>
                                  <span className="text-slate-800">₹6,200</span>
                                </div>
                              </div>

                              <div className="border-t border-slate-100 pt-0.5 text-[8px] font-semibold text-slate-500 space-y-0.5">
                                <div className="flex justify-between">
                                  <span>GSTIN</span>
                                  <span className="text-slate-600">29AAFCA8239F</span>
                                </div>
                                <div className="flex justify-between font-bold text-blue-600 border-t border-slate-50 pt-0.5">
                                  <span>Total Due</span>
                                  <span>₹19,470.00</span>
                                </div>
                              </div>
                  ...
                  <div className="lg:col-span-6 w-full">
                    <form onSubmit={handleRegisterSubmit} className="space-y-4 text-left">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1.5" htmlFor="reg-fullName-corp">
                            Full Name
                          </label>
                          <input
                            type="text"
                            id="reg-fullName-corp"
                            name="fullName"
                        ...
                        <div>
                          <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1.5" htmlFor="reg-phone-corp">
                            Phone Number
                          </label>
                          <input
                            type="tel"
                      ...
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1.5" htmlFor="reg-email-corp">
                          Email Address
                        </label>
                        <input
                          type="email"
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1.5" htmlFor="reg-password-corp">
                            Password
                          </label>
                          <input
                            type="password"
                            id="reg-password-corp"
                            name="password"
                        ...
                        <div>
                          <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1.5" htmlFor="reg-confirmPassword-corp">
                            Confirm Password
                          </label>
                          <input
                            type="password"
                            id="reg-confirmPassword-corp"
                      ...
                      <div className="text-center text-xs text-slate-500 font-medium pt-2">
                        ...
                        <button
                          type="button"
                          onClick={() => handleTabToggle(true)}
                          className="font-bold text-blue-600 hover:text-blue-700 transition duration-200 ml-1 hover:underline cursor-pointer bg-transparent border-none"
                        >
                          Sign in
                        </button>
                      </div>
